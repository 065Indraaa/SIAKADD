...
function SiswaOverview() {
  ...
  const statsCard = [
    { title: 'Kelas', value: user?.className || 'Belum Diatur', sub: 'Tahun Ajaran Aktif', icon: BookOpen, accent: 'text-blue-500', bg: 'bg-blue-500/10' },
    { title: 'Rata-rata Nilai', value: stats.rataNilai, sub: 'Semester Aktif', icon: TrendingUp, accent: 'text-emerald-500', bg: 'bg-emerald-500/10' },
    { title: 'Kehadiran', value: stats.kehadiran, sub: 'Tingkat Presensi', icon: CheckCircle, accent: 'text-violet-500', bg: 'bg-violet-500/10' },
    { title: 'Prestasi', value: stats.prestasi.toString(), sub: 'Total Pencapaian', icon: Award, accent: 'text-amber-500', bg: 'bg-amber-500/10' },
  ];
      ...
      try {
        if (user?.siswaId) {
          ...
          const jadwalHariIni = (jadwal || []).filter(
            (j: any) => j.hari?.toLowerCase() === hariIni.toLowerCase(),
          );
        ...
        <div className="flex items-center gap-2 px-4 py-2.5 scola-card rounded-xl">
          ...
          <div>
            ...
            <p className="text-sm font-semibold text-foreground mt-0.5">
              {currentTahunAjaran()} · {currentSemester()}
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="lg:col-span-2 scola-card p-0 overflow-hidden">
          ...
          <div className="p-4 space-y-2">
            {[
              ...
              ...(user?.className?.includes('10') || user?.className?.startsWith('X-') ? [
              ...
              <Link
                key={i}
                to={item.href}
                className="flex items-center gap-3 px-4 py-3.5 rounded-xl bg-muted/50 hover:bg-muted border border-border transition-all group"
              >
