          ...
          <div key={i} className="scola-stat-card p-5">
            <div className="flex items-center justify-between mb-4">
              ...
              {loading ? <Loader2 className="w-3 h-3 animate-spin text-muted-foreground" /> : (
                stat.trend === 'up' && (
                  <span className="text-[11px] font-semibold text-emerald-600 dark:text-emerald-400 bg-emerald-500/10 px-2 py-0.5 rounded-full flex items-center gap-0.5">
      ...
      <div className="grid lg:grid-cols-5 gap-6">
        ...
        <div className="lg:col-span-3 scola-card p-0 overflow-hidden">
          <div className="flex items-center justify-between px-6 py-4 border-b border-border">
            <div className="flex items-center gap-2.5">
              ...
              <h3 className="text-sm font-semibold text-foreground">Pengguna Terdaftar Terbaru</h3>
            </div>
          ...
          <div className="divide-y divide-border">
            {[...recentGuru.map(g => ({ name: g.name, role: 'Guru', detail: g.nip || '-', email: g.email })),
              ...
              <div key={i} className="flex items-center gap-4 px-6 py-4 hover:bg-accent transition-colors">
                <div className="w-8 h-8 rounded-xl bg-muted flex items-center justify-center text-muted-foreground text-xs font-semibold flex-shrink-0">
                  {item.name[0]}
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm text-foreground font-semibold leading-snug">{item.name}</p>
                  <p className="text-[11px] text-muted-foreground mt-0.5">{item.email}</p>
                </div>
                <div className="text-[11px] text-muted-foreground flex items-center gap-1 flex-shrink-0 font-medium bg-muted px-2 py-1 rounded-lg">
                  {item.role} · {item.detail}
                </div>
              </div>
            ))}
            {recentGuru.length === 0 && recentSiswa.length === 0 && !loading && (
              <div className="py-8 text-center text-muted-foreground text-sm">Belum ada pengguna terdaftar.</div>
            )}
          </div>
        </div>
        ...
        <div className="lg:col-span-2 scola-card p-0 overflow-hidden">
          <div className="flex items-center justify-between px-6 py-4 border-b border-border">
            ...
            <span className="text-[11px] font-semibold bg-blue-500/10 text-blue-600 dark:text-blue-400 px-2 py-0.5 rounded-full">
